...
class Solution {
  ...
  topKFrequent(nums, k) {
      ...
      for (let num of nums) { // iterate through each num in the nums array
          if (freqObj[num]) { // if the num is in the object
      ...
      //      -> descending order 
      let sortedKeys = Object.keys(freqObj).sort((a, b) => freqObj[b] - freqObj[a]); // sort in descending -> higher frequencies come first
